...
import mx.itam.alpha.client.ui.GameFrame;
...
import mx.itam.alpha.jms.AsyncTopicReceiver;

/**
 * Orquesta el cliente: autentica por TCP, abre las suscripciones JMS
 * y mantiene la UI sincronizada con los eventos del juego.
 */
public class GameClientController implements GameFrame.Listener {
    ...
    private void authenticate(TcpRequest request) {
        try {
            ...
            // Primero se abre JMS y luego se habilita la UI para minimizar ventanas
            // donde el usuario ya puede jugar pero todavía no recibe monstruos/eventos.
            openSubscriptions();
    ...
    private void openSubscriptions() {
        ...
        monstersReceiver.start(payload -> {
            ...
            SwingUtilities.invokeLater(() -> {
                // El monsterId viaja después en el HIT para evitar aceptar golpes obsoletos.
    ...
    private void handleWinnerEvent(GlobalGameEvent event) {
        ...
        if (event.getType() == GlobalEventType.RESET) {
            // El RESET sincroniza el estado local del cliente con el inicio de la nueva ronda.
            frame.clearMonster();
            currentMonsterId = -1L;
            updateScoreboard(event.getScores());
            if (sessionInfo != null) {
